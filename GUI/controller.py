...
class Controller(Frame):
    def __init__(self, master, parent, *args, **kwargs):
        ...
        back_to_menu_button = Button(self,
                                     text='Menu',
                                     command=lambda i=0: parent.back_to_menu())

        start = Button(self,
                       text='Start',
                       command=lambda i=0: parent.running_algorithm.start())
        pause = Button(self,
                       text='Pause',
                       command=lambda i=0: parent.running_algorithm.pause())
        stop = Button(self,
                      text='Stop',
                      command=lambda i=0: parent.running_algorithm.stop())

        render_frame = Frame(self, bg=self['bg'])
        render_speed = Scale(render_frame,
                             orient=HORIZONTAL,
                             bg=self['bg'],
                             from_=1, to=500,
        ...
        pause.grid(row=2, column=0, sticky='nsew')
        stop.grid(row=3, column=0, sticky='nsew', pady=5)

        render_frame.grid(row=4, column=0, sticky='nsew')
        render_speed_label.grid(row=0, column=0, sticky='s')
        render_speed.grid(row=1, column=0, sticky='new', pady=5)
